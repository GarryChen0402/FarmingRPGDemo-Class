...
public class ItemCodeDescriptionDrawer : PropertyDrawer
{
    ...
    {
        // Using BeginProperty / EndProperty on the parent property means that perfab override logic works on the entire property.
        EditorGUI.BeginProperty(position, label, property);

        if(property.propertyType == SerializedPropertyType.Integer)
        {
            EditorGUI.BeginChangeCheck();// Start of check of changed values
            // Draw item code
            var newValue = EditorGUI.IntField(new Rect(position.x, position.y, position.width, position.height/2), label, property.intValue);

            //Draw item descirption
    ...
    private string GetItemDescription(int itemCode)
    {
        ...
        if (itemDetails != null) return itemDetails.itemDescription;
        return "";
    }
}
